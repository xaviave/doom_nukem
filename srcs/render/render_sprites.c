...
/*   Updated: 2019/05/06 12:43:32 by mel-akio    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
    ...
    while (++i < mem->level->nb_monsters)
        if (mem->level->monsters[i].sector == sect)
        {
            ...
            if (vec.mz1 >= 0)
            {
                ...
                vec.my = H * (mem->level->player.z - mem->level->sector[sect].h_floor) / vec.mz1 + H * 0.5 + mem->level->player.recoil;
                if (vec.mx > 0 && vec.mx < W)
                    put_img_to_img(mem, &mem->monster, vec.mx, vec.my, (H * 0.02) / mem->level->monsters[i].distance);
            }
        }
}
