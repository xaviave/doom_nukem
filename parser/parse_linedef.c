...
/*   Updated: 2019/01/18 14:50:49 by xamartin    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void		check_vertex(char *str, int *error)
{
	int			i;
	int			ok;
	int			tmp;
...
static void		check_nu(t_parse *parse, char *nu, int *error)
{
	...
	*error += !check_int(&nu[i]) ? 1 : 0;
	while (!(*error) && ++tmp < 3)
	{
		...
		v = parse->vertex;
		*error += (!(*error) && !check_int(&nu[i])
			&& id_ok(v, &nu[i])) ? 1 : 0;
	}
	check_vertex(&nu[i], error);
	if (!(*error))
		add_list_l(&parse->linedef, nu);
}

int				check_same_id(t_parse *parse, int error)
{
	t_plinedef	*tmp;
